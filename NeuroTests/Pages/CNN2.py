...
def main():
    ...
    net.add(cnn.Conv2d(8, 3, inputDepth=1, padding=1))
    net.add(cnn.ReLU())

    net.add(cnn.Conv2d(8, 3, inputDepth=8, padding=1))
    net.add(cnn.ReLU())

    net.add(cnn.MaxPool2d(size=(1, 52)))  # (4,152,1)

    net.add(cnn.Conv2d(1, 1, inputDepth=8))  # (1,152,1)

    net.add(cnn.Flatten())  # (152)
    net.add(cnn.Sigmoid())  # (152)
    ...
    for epoch in range(epochs):
        ...
        data.save(model=net, path=r"D:\Folders\tempModels", fileName=f"3page1{epoch}, {round(total_loss / (len(x_train)/batch_size),2)}")
