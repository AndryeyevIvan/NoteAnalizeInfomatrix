...
class nn:
    ...
    def save(self, path="", fileName="nnData"):
        import os
        import json
    ...
    def load(self, path=""):
        ...
        self.struct = model_data["struct"]
        self.actiFunc = model_data["actiFunc"]
        self.weights = model_data["weights"]
        self.biases = model_data["biases"]

    def StartLearning(self, epoch = 1, sv = 0, path = "", name = ""):
        for i in range(epoch):
            ...
            for j in range(len(self.inLayers)):
                if sv > 0:
                    if j % sv == 0:
                        self.save(path=path, fileName=f"{name}_sample_{j}")
                ...
                for k in range(len(self.struct[-1])):
                    er = (self.struct[-1][k] - self.corrAnswers[k]) ** 2
                    totalError += er
                    print(er, self.corrAnswers, j)
            self.epochErrors.append(totalError / len(self.inLayers))
            self.save(path=path, fileName=f"{name}_epoch_{i}_loss_{round(totalError / len(self.inLayers), 2)}")
...
class cnn:
    ...
    class MaxPool2d:
        def __init__(self, size=2, stride=1, input=[], output=[]):
            ...
            if isinstance(stride, (tuple, list)):
                self.strideH = stride[0]
                self.strideW = stride[1]
            else:
                self.strideH = stride
                self.strideW = stride
        ...
        def forward(self, input):
            ...
            outH = (inH - self.sizeH) // self.strideH + 1
            outW = (inW - self.sizeW) // self.strideW + 1
            self.input = input
            self.output = []
            out = []
                    ...
                    for j in range(outW):
                        ...
                        for y in range(self.sizeH):
                            for x in range(self.sizeW):
                                values.append(input[c][i * self.strideH + y][j * self.strideW + x])
        ...
        def backward(self, dOutput):
            ...
            for c in range(len(self.input)):
                ...
                for i in range(outH):
                    for j in range(outW):
                        grad = dOutput[c][i][j]
                        max_val = self.output[c][i][j]
                        stop = False
                        for y in range(self.sizeH):
                            if stop:
                                break
                            for x in range(self.sizeW):
                                inY = i * self.strideH + y
                                inX = j * self.strideW + x

                                if inY < inH and inX < inW:
                                    if self.input[c][inY][inX] == max_val:
                                        channel[inY][inX] += grad
                                        stop = True
                                        break
                out.append(channel)
            return out
    ...
    class Sigmoid:
        ...
        def sigmoid(self, x):
            """Безопасный Sigmoid для одного числа, чтобы избежать Overflow"""
            if x >= 0:
                z = math.exp(-x)
                return 1 / (1 + z)
            else:
    ...
    def focalLoss(pred, target, alpha=0.25, gamma=2.0):
        eps = 1e-9
        loss = 0.0
        for p, t in zip(pred, target):
    ...
    def focalLossDerivative(pred, target, alpha=0.25, gamma=2.0):
        eps = 1e-9
        grads = []
        ...
        for p, t in zip(pred, target):
            ...
            if t == 1:
                grad = alpha * ((1 - p) ** (gamma - 1)) * (
                        gamma * math.log(p) + (1 - p) / p
                )
            else:
                grad = (1 - alpha) * (p ** (gamma - 1)) * (
                        gamma * math.log(1 - p) - p / (1 - p)
                )
